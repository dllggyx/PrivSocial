        ...
        switch (info.tag) {
            ...
            case "remove":
                info.enc.forEach((data, targetIDSet) -> {
                    for (String targetID : targetIDSet) {
                        PublicKeyPair targetpkp = ServerStorage.tabServer.get(targetID).pkp;
                        byte[] sig = null;
                        MyMessage myMsg = new MyMessage(info, targetpkp, targetID, data, sig);
            ...
            case "add":
                ...
                for(Node n :sTree.leaves){
                    if(!n.isBlank) {
                        PublicKeyPair addPkp = ServerStorage.tabServer.get(n.ID).pkp;
                        SignKeyPair addskp = ServerStorage.tabServer.get(n.ID).skp;
                        IdentityKeys addIdentityKey = new IdentityKeys(addPkp,addskp);
                        MyMessage myMsg = new MyMessage(info, addPkp, n.ID, null, null);
                        msgMap.put(n.ID, myMsg);
                        WelcomeMsg wm = new WelcomeMsg(n.ID,addIdentityKey);
                        welcome.add(wm);
                    }else{
