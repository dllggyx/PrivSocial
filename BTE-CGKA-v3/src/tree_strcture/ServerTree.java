...
public class ServerTree {
    ...
    public void create(Group group, String senderID){
            ...
            if (key.equals(senderID)) {
                ...
                synchronized (ServerStorage.tabServer) {
                    if (ServerStorage.tabServer.get(key) == null)
                        ServerStorage.tabServer.put(key, new TabEntry(value.pkp.pk, value.skp.svk));
                    ServerStorage.tabServer.get(key).setKey(value.pkp.pk, value.skp.svk);
                }
            }
        }
    ...
    public void update(String senderID,byte[] senderPk,byte[] senderSvk){
        ...
        synchronized (ServerStorage.tabServer) {
            if (ServerStorage.tabServer.get(senderID) == null)
                ServerStorage.tabServer.put(senderID, new TabEntry(senderPk, senderSvk));
            ServerStorage.tabServer.get(senderID).setKey(senderPk, senderSvk);
        }
    }

    public int add(String senderID,String targetID,byte[] targetPk, byte[] targetSvk){
        //No sample() required
        ...
        for(Node leaf: leaves){
            if(leaf.isBlank){
                leaf.isBlank = false;
                leaf.setIdentityKeys(targetPk, targetSvk);
                leaf.ID = targetID;
                pos = leaf.pos;
                break;
            }
        ...
        synchronized (ServerStorage.tabServer) {
            if (ServerStorage.tabServer.get(targetID) == null)
                ServerStorage.tabServer.put(targetID, new TabEntry(targetPk,targetSvk));
            ServerStorage.tabServer.get(targetID).setKey(targetPk,targetSvk);
        }
        return pos;
    }

    public void remove(String senderID,byte[] senderKey,String targetID,byte[] targetPk,byte[] targetSvk){
        size = size - 1;
        ...
        synchronized (ServerStorage.tabServer) {
            if (ServerStorage.tabServer.get(senderID) == null)
                ServerStorage.tabServer.put(senderID, new TabEntry(targetPk,targetSvk));
            ServerStorage.tabServer.get(senderID).setKey(targetPk,targetSvk);
        }
...
//        }
    ...
    public AppendMsg changeTree(Information info){
        ...
        switch(info.tag){
            case "init":
                create(info.group,info.senderID);
                break;
            case "update":
                update(info.senderID,info.senderPk,info.senderSvk);
                break;
            case "add":
                ...
                int pos = add(info.senderID,targetID,targetKey.pkp.pk, targetKey.skp.svk);
            ...
            case "remove":
                ...
                remove(info.senderID,info.senderPk,targetID,targetKey.pkp.pk, targetKey.skp.svk);
